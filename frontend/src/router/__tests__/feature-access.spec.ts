...
describe('feature route guard', () => {
  ...
  it('redirects when loaded settings explicitly disable payment', async () => {
    appStore.cachedPublicSettings = { payment_enabled: false }
    appStore.publicSettingsLoaded = true

    const { navigation, next } = runGuard({ requiresPayment: true }, '/feature')
    await navigation
  ...
  it('revalidates an explicitly disabled risk-control cache entry before redirecting', async () => {
    authStore.isAdmin = true
    appStore.cachedPublicSettings = { risk_control_enabled: false }
    appStore.publicSettingsLoaded = true
    appStore.fetchPublicSettings.mockResolvedValue({ risk_control_enabled: false })

    const { navigation, next } = runGuard({ requiresRiskControl: true }, '/feature')
    await navigation
