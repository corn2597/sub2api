export default {
  ...
  admin: {
    ...
    riskControl: {
      ...
      provider: '风控 Provider',
      providerLegacy: 'OpenAI Moderations（旧版）',
      providerLegacyHint: '使用 OpenAI /v1/moderations，对每次输入独立评分。',
      providerSessionAudit: 'OpenAI Responses 会话审计',
      providerSessionAuditHint: '使用 /v1/responses 审计同一会话的上下文，并按 session 黑名单拦截后续请求。',
      ...
      sessionAuditTitle: 'Responses 会话审计',
      sessionAuditHint: '这组配置独立于旧版 Moderations Key。默认只审计 anthropic_messages，避免审计请求自调用时递归。',
      auditBaseUrl: '审计 Base URL',
      auditPath: '审计路径',
      auditModel: '审计模型',
      auditTimeoutMs: '审计超时 (ms)',
      auditBlockConfidenceThreshold: '拦截置信度阈值',
      sessionAuditIntervalSeconds: '同会话复审间隔（秒）',
      sessionBlacklistTTLSeconds: '会话黑名单 TTL（秒，0=永久）',
      auditMaxInputChars: '审计最大输入字符数',
      sessionAuditEnabledProtocols: '启用协议',
      sessionAuditEnabledProtocolsHint: '每行或逗号分隔。线上建议先只保留 anthropic_messages，不要加入 openai_responses 以避免自调用递归。',
      auditApiKeys: '审计 API Keys',
      auditApiKeysPlaceholder: '新增审计 API Key，每行一个；保存后会追加到已保存 Key',
      auditApiKeysPlaceholderReplace: '覆盖保存审计 API Key，每行一个；保存后会替换全部已保存 Key',
      auditPromptTemplate: '自定义审计提示词',
      auditPromptTemplatePlaceholder: '留空使用内置 Claude/Anthropic 会话审计提示词。',
      auditFailClosed: '审计异常时拒绝请求',
      auditFailClosedHint: '开启后审计模型、Key 或上游异常会直接拦截请求。线上首次启用建议先关闭。',
      auditModelRequired: '启用 Responses 会话审计需要填写审计模型',
      auditApiKeyRequired: '启用 Responses 会话审计需要至少 1 个审计 API Key',
      auditProtocolsRequired: '启用 Responses 会话审计需要至少 1 个协议',
