export default {
    settings: {
      ...
      openaiEgress: {
        title: 'OpenAI Egress Runtime',
        description: 'An optional Bun sidecar handles OpenAI OAuth, HTTP/SSE, and WebSocket egress. When disabled, the existing Go transport remains unchanged and users see no difference.',
        enabled: 'Enable OpenAI sidecar egress',
        enabledHint: 'Only internal outbound calls use the sidecar; admin APIs, account management, and the user experience stay unchanged.',
        http: 'HTTP / SSE',
        httpHint: 'Responses, Chat Completions, image, and other HTTP egress',
        ws: 'WebSocket',
        wsHint: 'OpenAI Responses WebSocket bridge',
        oauth: 'OAuth credentials',
        oauthHint: 'Authorization-code exchange and refresh-token egress',
        fallback: 'Fall back to the existing path',
        fallbackHint: 'Enabling this allows OpenAI traffic to bypass Bun when the sidecar is unavailable; disable it for fail-closed routing.',
        allowProxy: 'Use account proxies through Bun',
        allowProxyHint: 'Pass each configured account proxy to Bun for HTTP and WebSocket egress so the public connection still originates in Bun.',
        endpoint: 'Sidecar endpoint',
        endpointHint: 'Read from gateway.openai_egress.base_url; loopback is the default.',
        security: 'Shared secret',
        secretConfigured: 'Shared secret configured',
        secretMissing: 'Shared secret missing',
        securityHint: 'The secret is read only from server configuration and is never echoed in the UI.',
        fallbackNotice: 'Direct bypass is enabled: OAuth, HTTP/SSE, or WebSocket traffic may use the Go direct/proxy path when Bun is unavailable.',
        proxyDisabledNotice: 'Account proxy forwarding is disabled. Proxied accounts fail closed in strict mode instead of bypassing Bun.',
        notChecked: 'Sidecar status has not been checked',
        statusDisabled: 'Disabled',
        statusReady: 'Sidecar connected',
        statusUnavailable: 'Waiting for sidecar',
        test: 'Check connection',
        saved: 'OpenAI egress settings saved',
        testPassed: 'Sidecar connection is healthy',
      },
      features: {
        channelMonitor: {
