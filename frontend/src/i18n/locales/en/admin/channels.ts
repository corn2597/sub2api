export default {
    ...
    riskControl: {
      ...
      modePreBlockDesc: 'Synchronously reviews all user input in the current payload before every request and rejects hits immediately.',
      modeAsyncBlock: 'Async Block',
      modeAsyncBlockDesc: 'Requests pass through first; explicit sessions are always reviewed on the first turn, receive a 5 minute allow window after a clean result, and are blacklisted after an async hit.',
      modeObserve: 'Observe Only',
      modeObserveDesc: 'Requests pass through while all user input in the current payload is queued for async review; hits are recorded, notified, and counted.',
      modeOff: 'Off',
      ...
      sampleRate: 'Fallback Sampling Rate',
      ...
      blockedKeywordsSyncHint: 'Keyword blocking runs synchronously in both "Pre-block" and "Async Block" modes.',
      blockedKeywordsModeWarning: 'Current mode is "{mode}". Keyword blocking will not run until you switch to "Pre-block" or "Async Block" mode.',
      ...
      keywordModeKeywordOnlyAsyncBlockNotice: 'Async-block + keyword-only strategy: the first turn of an explicit session is still reviewed.',
      keywordModeKeywordOnlyAsyncBlockDesc: 'Keyword hits are still blocked synchronously. If a request misses the keywords but carries an explicit session, the first turn is still reviewed asynchronously and receives a 5 minute allow window after a clean result. Only non-explicit fallback requests skip the upstream audit.',
      ...
      action: {
        ...
        asyncBlock: 'Async Hit + Session Blacklisted',
