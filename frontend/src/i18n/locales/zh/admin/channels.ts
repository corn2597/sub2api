export default {
    ...
    riskControl: {
      ...
      mode: '全局模式',
      modePreBlock: '前置拦截',
      modePreBlockDesc: '每次请求先同步审核当前 payload 中的全部 user input，命中后立即拒绝请求。',
      modeAsyncBlock: '异步拉黑',
      modeAsyncBlockDesc: '请求先放行；显式 session 首条必审，通过后 5 分钟内放行，异步命中后拉黑该 session，后续本地秒拦。',
      modeObserve: '仅观察',
      modeObserveDesc: '请求直接放行，当前 payload 中的全部 user input 进入异步审核队列；命中后只记录、通知和按规则累计。',
      modeOff: '关闭',
      ...
      blockedKeywordsSyncHint: '关键词拦截在「前置拦截」和「异步拉黑」模式下都会同步生效。',
      blockedKeywordsModeWarning: '当前为「{mode}」模式，关键词拦截不会生效；请切换到「前置拦截」或「异步拉黑」模式后再保存关键词。',
      ...
      keywordModeKeywordOnlyAsyncBlockNotice: '当前为「异步拉黑 + 仅关键词」策略：显式 session 首条仍会送审。',
      keywordModeKeywordOnlyAsyncBlockDesc: '关键词命中仍会同步拦截；若未命中但请求带显式 session，首条依然会异步送审，通过后进入 5 分钟放行窗口。只有非显式 fallback 请求才会跳过上游审计。',
