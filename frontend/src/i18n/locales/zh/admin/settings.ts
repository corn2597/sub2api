export default {
    settings: {
      ...
      openaiEgress: {
        title: 'OpenAI 出站运行时',
        description: '可选的 Bun sidecar 负责 OpenAI OAuth、HTTP/SSE 与 WebSocket 出站。关闭时完全沿用现有 Go 出站，用户端接口和 OAuth 管理流程不变。',
        enabled: '启用 OpenAI sidecar 出站',
        enabledHint: '启用后仅内部出站请求经过 sidecar；管理员 API、账号管理和用户体验保持不变。',
        http: 'HTTP / SSE',
        httpHint: 'Responses、Chat Completions 和图片等 HTTP 出站',
        ws: 'WebSocket',
        wsHint: 'OpenAI Responses WebSocket 出站桥接',
        oauth: 'OAuth 凭据',
        oauthHint: '授权码交换与 refresh token 出站',
        fallback: 'sidecar 故障时回退直连',
        fallbackHint: '开启会允许 sidecar 故障时绕过 Bun 直连 OpenAI；关闭则严格 fail-closed。',
        allowProxy: '通过 Bun 使用账号代理',
        allowProxyHint: '账号配置代理时，将代理地址交给 Bun 的 HTTP 与 WebSocket 客户端，公网流量仍由 Bun 发出。',
        endpoint: 'Sidecar 地址',
        endpointHint: '来自 gateway.openai_egress.base_url；默认绑定本机回环地址。',
        security: '共享密钥',
        secretConfigured: '已配置共享密钥',
        secretMissing: '未配置共享密钥',
        securityHint: '密钥只从服务端配置读取，不会在管理界面回显。',
        fallbackNotice: '当前允许绕过 Bun：sidecar 故障时 OAuth、HTTP/SSE 或 WS 可能回到 Go 直连/代理链路。',
        proxyDisabledNotice: '账号代理转发已关闭。配置了代理的账号在严格模式下会失败关闭，不会绕过 Bun 直连。',
        notChecked: '尚未检查 sidecar 状态',
        statusDisabled: '已关闭',
        statusReady: 'sidecar 已连接',
        statusUnavailable: '等待 sidecar',
        test: '检查连接',
        saved: 'OpenAI 出站设置已保存',
        testPassed: 'sidecar 连接正常',
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
