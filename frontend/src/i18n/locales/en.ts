export default {
  ...
  admin: {
    ...
    riskControl: {
      ...
      providerLegacy: 'OpenAI Moderations (legacy)',
      providerLegacyHint: 'Uses OpenAI /v1/moderations to score each input independently.',
      providerSessionAudit: 'OpenAI Responses Session Audit',
      providerSessionAuditHint: 'Uses /v1/responses to audit session context and block future requests via a session blacklist.',
      ...
      sessionAuditTitle: 'Responses Session Audit',
      sessionAuditHint: 'This configuration is separate from legacy Moderations keys. By default only anthropic_messages is audited to avoid recursive self-audit calls.',
      auditBaseUrl: 'Audit Base URL',
      auditPath: 'Audit Path',
      auditModel: 'Audit Model',
      auditTimeoutMs: 'Audit Timeout (ms)',
      auditBlockConfidenceThreshold: 'Block Confidence Threshold',
      sessionAuditIntervalSeconds: 'Same-session Recheck Interval (seconds)',
      sessionBlacklistTTLSeconds: 'Session Blacklist TTL (seconds, 0=permanent)',
      auditMaxInputChars: 'Audit Max Input Characters',
      sessionAuditEnabledProtocols: 'Enabled Protocols',
      sessionAuditEnabledProtocolsHint: 'One per line or comma-separated. For production, keep only anthropic_messages first; do not add openai_responses if using self-call.',
      auditApiKeys: 'Audit API Keys',
      auditApiKeysPlaceholder: 'Add audit API keys, one per line. They will be appended on save.',
      auditApiKeysPlaceholderReplace: 'Replace audit API keys, one per line. Stored keys will be replaced on save.',
      auditPromptTemplate: 'Custom Audit Prompt',
      auditPromptTemplatePlaceholder: 'Leave empty to use the built-in Claude/Anthropic session audit prompt.',
      auditFailClosed: 'Fail closed on audit errors',
      auditFailClosedHint: 'When enabled, missing model/key or audit upstream failures block the request. Keep off for the first production rollout.',
      auditModelRequired: 'Responses session audit requires an audit model',
      auditApiKeyRequired: 'Responses session audit requires at least 1 audit API key',
      auditProtocolsRequired: 'Responses session audit requires at least 1 protocol',
